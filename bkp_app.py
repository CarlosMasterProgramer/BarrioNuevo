        ...
        if 'additional_images' in request.files:
            for image in request.files.getlist('additional_images'):
                if image.filename != '':
                    ...
                    property_image = PropertyImage(property_id=new_property.id, image_url=additional_image)
    ...
    if request.method == 'POST':
        ...
        if 'additional_images' in request.files:
            for image in request.files.getlist('additional_images'):
                if image.filename != '':
                    additional_image = photos.save(image)
                    property_image = PropertyImage(property_id=property.id, image_url=additional_image)
                    db.session.add(property_image)
